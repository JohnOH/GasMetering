...
unsigned int TimeOut = 60 * 110;        // Initial timeout if interrupts don't happen approx 1 hour
...
digitalWrite(DIO2, HIGH);  //use the internal pullup resistor
  ...
  pulse++;
  if (pulse < 4) return; // Four pulses before meter increments
  pulse = 0;
...
void setup () {
  ...
  pinMode(DIO2, OUTPUT);      //set the  DEBUG pin to output
digitalWrite(DIO2, HIGH);  //use the internal pullup resistor
...
void loop () {
      ...
      while (ms < TimeOut)   // Note ms is not an accurate millsecond since using Prescaler Max & Min values. 220=min 13320=hour
      {
        ...
        sleep_mode();
///// Wake up here? ///////// 
digitalWrite(DIO2, LOW);  //use the internal pullup resistor
        wdt_reset();      // We wake very often anyway
...
/////
  ...
  if (payload.vcc < 61)
    {
      TimeOut = 400 * 24;   // Low voltage: reduce I'm alive message frequency
