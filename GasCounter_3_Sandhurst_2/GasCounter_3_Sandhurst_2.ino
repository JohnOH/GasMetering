...
unsigned int GasMeterIncrement; // Reported gas meter pulse count                         //
unsigned int ElecMeterIncrement; // ditto electricity
...
volatile boolean GasMeterChanged = false;
volatile boolean ElecMeterChanged = false;
volatile boolean WDTwake = false;
volatile int pulse0 = 1;          // Always assume meter increments at startup
volatile int pulse1 = 1;          // Always assume meter increments at startup
...
//  WaitTime = now + debounce;  /* When using "set_sleep_mode(SLEEP_MODE_PWR_DOWN);" */
  pulse0++;
  if (pulse0 < 2) return; // Two pulses before meter increments
  pulse0 = 0;
  payload.GasMeterIncrement++;   // Add one to meter count
  payload.interval = (now - previous) / 1000;
  GasMeterChanged = true;
  previous = now;
}
ISR (PCINT1_vect) { 
  unsigned long now;      // The instant!
  now = pseudoTime;
  pulse1++;
  if (pulse1 < 2) return; // Two pulses before meter increments
  pulse1 = 0;
  payload.ElecMeterIncrement++;   // Add one to meter count
  ElecMeterChanged = true;
#ifdef DEBUG
  digitalWrite(AIO1, HIGH);  // sets the LED on
#endif
}
...
unsigned int getCT(void)   /* Reading value of Current Transformer (CT) */
 {
  // Select bandgap as ADC input 
  bitClear(PRR, PRADC);  /* power up the ADC */
  ADMUX =  B10000011;  /* 1.1V reference ADC3 */
  /* We are probably running at slow speed already
  ADCSRA = bit(ADPS2) | bit(ADPS1) | bit(ADPS0); // Set ADC prescaler divide by 128
  So the above is probably not required */
  ADCSRA |= bit(ADEN); // enable the ADC
  delay(2);   //Allow ADC to settle
  ADCSRA |= bit(ADSC);        // Start a measurement
  while (ADCSRA & bit(ADSC)); // Loop until complete 
...
void setup () {
  ...
  pinMode(PB0, INPUT);      //set the pin to input IDX
  PCMSK0 |= (1<<PCINT2); //  tell pin change mask to listen to (DIO1)
  PCMSK1 |= (1<<PCINT8); //  tell pin change mask to listen to (IDX/PB0)
  GIMSK  |= (1<<PCIE0); // enable PCINT0 interrupt in the general interrupt mask
  GIMSK  |= (1<<PCIE1); // enable PCINT1 interrupt in the general interrupt mask
...
#ifdef DEBUG
  pinMode(AIO1, INPUT);      //set the pin to input
  digitalWrite(AIO1, HIGH);  // sets the LED on
  pinMode(AIO2, INPUT);      //set the pin to input
  digitalWrite(AIO2, HIGH);  // sets the LED on
#endif
  rf12_setWatchdog(8000); // 8 seconds
}
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void loop () {
      setPrescaler(8); // div 256, i.e. 1 or 8 MHz div 256 (8) = 32kHz
      bitSet (PRR, PRUSI);   // Power down USI
...
///// Wake up here? ///////// 
        ...
        if (rf12_watchdogFired()) 
        {
          ...
          rf12_setWatchdog(8000); // 8 seconds
#ifdef DEBUG
  digitalWrite(AIO2, WDTwake);  // sets the LED
#endif
          ms++;
          WDTwake = !WDTwake; 
//          payload.getCT = getCT();   // Grab value of the Current Transformer voltage //
        }

        if ((GasMeterChanged || ElecMeterChanged) && NextTime < pseudoTime)      // Has either meter changed? // Impose a minimum delay between transmissions
          {
             GasMeterChanged = false; // Reset for next pass
             ElecMeterChanged = false; // Reset for next pass   // Why not share a flag?
             if (PowerOK)
             {
               payload.count++;
               bitClear (PRR, PRUSI);   // Power up USI
               ...
               rf12_sendWait(2);
               rf12_sleep(RF12_SLEEP);
//               NextTime = pseudoTime + 60000; // Wait a minimum time before next transmitting again //
//               wdt_reset();
               payload.vcc = getVcc();
               setPrescaler(8); // div 256, i.e. 1 or 8 MHz div 256 (8) = 32kHz
...
/////
  ...
  if (payload.vcc < 61)
    {
      TimeOut = 416 * 24;   // Low voltage: reduce I'm alive message frequency
