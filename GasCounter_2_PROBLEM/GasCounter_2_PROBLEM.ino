...
#define AIO2 9 // d9
#define DIO2 10 // d10
#define debounce 5 // debounce time need to factor in prescaler slowdown.
volatile unsigned long WaitTime; // stored time of interrupt
volatile unsigned long previous; // mills value at previous interrupt
unsigned int PreviousMeterPulse;
unsigned int vmin, vmax;

//
struct payload{
byte count; // packet count
unsigned int MeterPulse; // Meter pulse count
unsigned int interval;   // time between interrupts (ms)
signed int temp; //getAvrTemp value
unsigned long vcc; //getVcc                   // 1.0V = 0, 1.8V = 40, 3.3V = 115, 5.0V = 200, 6.0V = 250
} payload;
//
ISR (PCINT0_vect) { 
  volatile unsigned long WaitTime; // stored time of interrupt
  volatile unsigned long previous; // mills value at previous interrupt
  volatile unsigned long now;      // The instant!
  now = millis();
...
void setup () {
  ...
  PreviousMeterPulse = 0;
  payload.MeterPulse = 0;
  payload.count = 0;
...
// 13 = 9mins 34sec
// 14 = 34mins 38sec
  nextTime = millis() >> 8; // 13;  // Watchdog every 18 hours
  if (PreviousMeterPulse == payload.MeterPulse) // Has there been a DIO1 interrupt?
  {                                  // No
  ...
  lastTime = nextTime;
  setPrescaler(0); // div 1, i.e. speed up to 8 MHz
  PreviousMeterPulse = payload.MeterPulse;
  payload.count++;
