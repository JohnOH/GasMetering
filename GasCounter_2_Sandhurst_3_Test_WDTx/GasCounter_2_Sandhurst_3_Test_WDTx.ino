...
volatile bool even = false;        // Only even numbered clicks increment the counter.
bool PowerOK = true;
volatile bool MeterChanged = false;
volatile bool WatchDogInterrupted = false;
unsigned int ms = 65535;          // Actually 256ms per unit with Prescaler at maximum.
unsigned int TimeOut = 10;   // Initial timeout if interrupts don't happen
//
//Watchdog timeout ISR
ISR (WDT_vect) {
  payload.debug++;
  WatchDogInterrupted = true;  // set global flag
  bitSet (WDTCSR, WDIE);
}
ISR (PCINT0_vect) { 
...
void loop () {
      ...
      while (ms < TimeOut)   // Note ms is not an accurate millsecond since using Prescaler Max & Min values. 220=min 13320=hour
      {
        ...
        sleep_mode();                         // Expecting to sleep here for 8 seconds
        payload.debug2++;
///// Wake up here? /////////        
