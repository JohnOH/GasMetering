...
static byte myNodeID= 18;       // node ID used for this unit
#include <JeeLib.h>
#include <avr/sleep.h>
...
void loop () {
  ...
  while (ms < TimeOut)   // Note ms is not an accurate millsecond since using Prescaler Max & Min values. 220=min 13320=hour
  {
      ...
      if (PreviousMeterClick != MeterClick)
      {   
        if (even)   // Alternate clicks increment the counter
        {
        ...
        even = !even; // Toggle for next click
      }
    ms++;
  }
  ms = 0;
